...
impl<'a> Resolver<'a> {
    ...
    fn resolve_config(&mut self, parent_crate: CrateId, cfg: &Config) -> Result<(), CliError> {
        ...
        for (dep_pkg_name, pkg_src) in &cfg.dependencies {
        ...
        for (dir_path, (crate_id, dep_meta)) in &self.cached_packages {
            ...
            new_res.resolve_config(*crate_id, &dep_meta.cfg)?;
        }
        Ok(())
    }
    ...
    pub fn resolve_git_dep(url: &str, tag: &str) -> Result<PathBuf, CliError> {
        match git::clone_git_repo(url, tag) {
            Ok(path) => Ok(path),
